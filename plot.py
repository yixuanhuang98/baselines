...
for seed in seeds:
    for reward_scale in reward_scales:
        ...
        lc_rewards = np.loadtxt('./baselines/ppoc/data/'+env_id+'_s'+str(seed)+'_r'+str(reward_scale)+'_rew.txt')* (1 / reward_scale)
        lc_ts = np.loadtxt('./baselines/ppoc/data/'+env_id+'_s'+str(seed)+'_r'+str(reward_scale)+'_ts.txt')
