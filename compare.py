...
def plot2(seeds, reward_scale, alg, env_id, e):
    ...
    for seed in seeds:
        ...
        rw = np.loadtxt('./baselines/'+alg+'/base_data/'+env_id+'_s'+str(seed)+'_r'+str(reward_scale)+'_rew'+e+'.txt') * (1 / reward_scale)
        if rw.size > max_len:
            max_len = rw.size
            tss = np.loadtxt('./baselines/'+alg+'/base_data/'+env_id+'_s'+str(seed)+'_r'+str(reward_scale)+'_ts'+e+'.txt')
        rws.append(rw)
...
env_ids = ["Walker2d-v2", "Swimmer-v2"]
seeds = range(1,16)
reward_scale = 1.0
alg = 'fcn'
