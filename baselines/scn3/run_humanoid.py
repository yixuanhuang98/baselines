...
def train(env_id,num_timesteps, seed, model_path=None, ratio=0.1):
    from baselines.scn3 import mlp_policy, pposgd_simple
...
def main():
    ...
    if not args.play:
        ...
        np.savetxt('./baselines/scn3/data/'+args.env+'_s'+str(args.seed)+'_stds.txt', np.asarray(stds))
        np.savetxt('./baselines/scn3/data/'+args.env+'_s'+str(args.seed)+'_means.txt', np.asarray(means))
