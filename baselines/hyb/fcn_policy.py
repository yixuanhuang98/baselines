...
class FcnPolicy(object):
    ...
    def _init(self, ob_space, ac_space, num_actors, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        with tf.variable_scope('pol'):
            ...
            scn_last_out = obz
            for i in range(num_hid_layers):
                scn_last_out = tf.nn.tanh(tf.layers.dense(scn_last_out, 16, name='fc%i'%(i+1), kernel_initializer=U.normc_initializer(1.0)))
            ...
            if gaussian_fixed_var and isinstance(ac_space, gym.spaces.Box):
                for i in range(num_actors-1):
                    actors.append(tf.layers.dense(obz,pdtype.param_shape()[0]//2,name="final%i"%(i+1),kernel_initializer=U.normc_initializer(0.01)))
                ...
                # scn actor
                a_N = tf.layers.dense(scn_last_out, pdtype.param_shape()[0]//2, name="a_N", kernel_initializer=U.normc_initializer(0.01), use_bias=False)
                a_L = tf.layers.dense(obz,pdtype.param_shape()[0]//2,name="a_L",kernel_initializer=U.normc_initializer(0.01))
                mean = tf.add(a_L,a_N, name="final_scn")
                logstd = tf.get_variable(name="logstd_scn", shape=[1, pdtype.param_shape()[0]//2], initializer=tf.zeros_initializer())
                pdparams.append(tf.concat([mean, mean * 0.0 + logstd], axis=1))
