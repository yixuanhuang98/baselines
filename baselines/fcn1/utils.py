...
class Dataset(object):
    ...
    def delta_state_mean(self):
        return np.mean(np.array(self._next_states) - np.array(self._truth_states), axis=0)

    @property
    def delta_state_std(self):
        return np.std(np.array(self._next_states) - np.array(self._truth_states), axis=0)

    ###################
    ### Adding data ###
    ###################

    def add(self, state, truth_state, action, next_state, reward, done):
        """
        ...
        self._truth_states.append(np.ravel(truth_state))
    ...
    def rollout_iterator(self):
        ...
        truth_states = np.asarray(self._truth_states)
        ...
        for end_idx in end_indices:
            ...
            yield states[indices], truth_states[indices], actions[indices], next_states[indices], rewards[indices], dones[indices]
            start_idx = end_idx
