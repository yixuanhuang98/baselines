...
class ModelBasedPolicy(object):
    ...
    def __init__(self,
        ...
        self._orn_state_dim = env.observation_space.shape[0] - 2
        ...
        self._sess, self._state_ph, self._truth_state_ph, self._action_ph, self._next_state_ph,\
    ...
    def _setup_placeholders(self):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        state_ph = tf.placeholder(shape=[None, self._orn_state_dim], name='state', dtype=tf.float32)
        truth_state_ph = tf.placeholder(shape=[None, self._state_dim], name='truth_state', dtype=tf.float32)
        ...
        return state_ph, truth_state_ph ,action_ph, next_state_ph

    def _dynamics_func(self, state, truth_state, action, reuse):
        """
        ...
        # print(state)
        # print('state_mean')
        # print(self._init_dataset.state_mean)
        state_norm = baselines.fcn1.utils.normalize(state, self._init_dataset.state_mean, self._init_dataset.state_std)
        action_norm = baselines.fcn1.utils.normalize(action, self._init_dataset.action_mean, self._init_dataset.action_std)
        input_layer = tf.concat([state_norm, action_norm], axis=1)
        delta_pred_norm = baselines.fcn1.utils.build_mlp(
            input_layer, 
            self._state_dim, 
            scope='dynamics_func',
        ...
        delta_pred = baselines.fcn1.utils.unnormalize(delta_pred_norm, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        next_state_pred = truth_state + delta_pred

        return next_state_pred

    def _setup_training(self, state_ph, truth_state_ph, next_state_ph, next_state_pred):
        """
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        diff = next_state_ph - truth_state_ph
        diff_pred = next_state_pred - truth_state_ph
        diff_norm = baselines.fcn1.utils.normalize(diff, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        diff_pred_norm = baselines.fcn1.utils.normalize(diff_pred, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
    ...
    def _setup_action_selection(self, state_ph):
        ...
        for t in range(self._horizon):
            next_states = self._dynamics_func(states[:,2:], states, actions[:, t, :], True)
    ...
    def _setup_graph(self):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        state_ph, truth_state_ph, action_ph, next_state_ph = self._setup_placeholders()
        next_state_pred = self._dynamics_func(state_ph, truth_state_ph, action_ph, False)
        loss, optimizer = self._setup_training(state_ph, truth_state_ph, next_state_ph, next_state_pred)
        ### PROBLEM 2
        ### YOUR CODE HERE
        best_action = self._setup_action_selection(truth_state_ph)

        sess.run(tf.global_variables_initializer())

        return sess, state_ph, truth_state_ph, action_ph, next_state_ph, \
                next_state_pred, loss, optimizer, best_action

    def train_step(self, states, truth_states, actions, next_states):
        """
        ...
        feed_dict = {
            ...
            self._truth_state_ph: truth_states,
    ...
    def predict(self, state, truth_state, action):
        """
        ...
        assert np.shape(state) == (self._orn_state_dim,)
        ...
        feed_dict = {
            ...
            self._truth_state_ph: [truth_state],
    ...
    def get_action(self, state):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        best_action = self._sess.run(self._best_action, feed_dict={self._truth_state_ph:[state]})
